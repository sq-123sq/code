...
void push_num(task_num* p,double data){
    if(p==NULL)
    {
        printf("空栈\n");
        return;
    ...
    p->top++;
    p->data[p->top]=data;//入栈
}
void push_op(task_op* p,char data){
    if(p==NULL)
    {
        printf("空栈\n");
        return;
...
double calculate(double a,char c,double b)//单次计算
{
    switch (c)
    {
    case '+':
